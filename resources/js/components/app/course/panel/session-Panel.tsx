...
export function SessionPanel({ courseId, currentUrl }: Props) {
    ...
    if (!forum) return <div>Forum for {courseId} is not found</div>;
                    ...
                    {sessionLinks.map((item, index) => {
                        ...
                        return (
                            <a key={index} href={item.href} className="">
                                <div
                                    className={`mb-3 items-center rounded-lg border px-3 py-3 text-sm font-medium shadow-sm transition duration-200 ${
                                        isActive
